        ...
        html! {
            <div class="w-full mx-auto">
                <div class="rounded transition-all duration-500 ease-in-out">
                    <div class={if self.open {"bg-background-300 rounded-t"} else {"bg-background-200 rounded-t"}}>
                        <button  class="flex items-center justify-between w-full p-3 font-medium rtl:text-left rounded-t hover:bg-background-300" onclick={ctx.link().callback(|_| Form::Toggle)}>
                    ...
                    <div class={if self.open { "max-h-[1000px] transition-all duration-500 ease-in-out overflow-hidden" } else { "max-h-0 transition-all duration-500 ease-in-out overflow-hidden" }}>
                        <p class="w-full p-4 text-text-950 text-base bg-background-300 rounded-b">
